...
const FAQ_ITEMS = [
  ...
  },
  {
    ...
    answer:
      "네, 화상 또는 전화를 통한 비대면 상담과 센터 방문 대면 상담 모두 가능합니다. 신청 시 원하시는 방식을 선택해주세요.",
  },
  {
  ...
  return (
    <div>
      <section className="bg-gradient-to-b from-teal-50 to-white">
        <div className="mx-auto max-w-5xl px-4 py-20 text-center sm:px-6">
          ...
          <h1 className="mt-3 text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl">
            나를 이해하는 첫걸음,
            <br className="hidden sm:block" /> 전문가와 함께하는 심리검사
          </h1>
          <p className="mx-auto mt-4 max-w-xl text-base leading-7 text-slate-600">
            성격, 진로적성, 정서·스트레스까지. 표준화된 심리검사와 1:1 해석상담으로
            나를 더 깊이 이해하는 시간을 가져보세요.
          </p>
          <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
            <Link
              href="/apply"
              className="w-full rounded-full bg-teal-600 px-6 py-3 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-teal-700 sm:w-auto"
            >
              상담 신청하기
            </Link>
            <Link
              href="#tests"
              className="w-full rounded-full border border-slate-300 px-6 py-3 text-sm font-semibold text-slate-700 transition-colors hover:bg-slate-50 sm:w-auto"
            >
              검사 종류 살펴보기
            </Link>
          </div>
        ...
        <div className="mt-8 grid gap-6 sm:grid-cols-2">
          {COUNSELORS.map((counselor) => (
            ...
            >
              ...
              <p className="mt-1 text-sm text-teal-700">{counselor.role}</p>
      ...
      <section className="mx-auto max-w-5xl px-4 py-16 text-center sm:px-6">
        ...
        <Link
          href="/apply"
          className="mt-6 inline-flex items-center justify-center rounded-full bg-teal-600 px-6 py-3 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-teal-700"
        >
          상담 신청하기
        </Link>
      </section>
